...
**⚠️ Before launch — revert the TEST-ONLY tournament-gate bypass** (commented out in `app/api/admin/tournaments/route.ts` + `app/admin/tournaments/page.tsx`; both marked `TEST ONLY (Phase 58)`). Tournament creation should require a Participating gym again.

### Phase 59 — Challenge Matches — DONE & browser-verified 2026-06-12 (NOT committed to git)
Full negotiation→waiver→approval→console→record flow. tsc + `npm run build` clean. **Additive `prisma db push` run** (shared cluster): `ChallengeMatch`, `ChallengeTermsHistory`, `GymWaiver`, `WaiverSignature` + `ChallengeStatus`/`WaiverKind` enums; `User.competeTournaments`/`acceptsChallenges`; `Gym.hostsInHouseChallenges`/`hostsOpenChallenges`. Dev restarted.
- **Opt-ins:** member toggles ("Compete in tournaments" / "Accept challenge matches") in `/settings` (Competition section); gym hosting toggles + visitor-waiver publish in `/admin/settings` (`ChallengeHostingForm`). Profile shows a **Challenge** button (next to Follow) when both viewer + target accept challenges, plus a **competition record** (W–L–D from completed challenges, `getCompetitionRecord`).
- **Lifecycle API:** `POST /api/challenges` (propose, one-active-per-pair, +14d expiry), `PATCH /api/challenges/[id]` (counter/accept/decline/withdraw with whose-turn enforcement via `lastActorId`), `POST .../sign` (typed-name waiver e-sign → `WaiverSignature`), `POST .../approve` (host gym admin → scheduled), `POST .../table` (spawn Phase 58 table, `challengeId` set). `lib/challenge.ts` = whose-turn helpers, `maybeAdvanceToGymPending` (accepted→gym_pending when no waiver or both signed), `waiverStateFor`, `applyChallengeResult`, `getCompetitionRecord`. Console **Finish** now also propagates to a challenge (winnerSide→winnerId, status=completed, notifies both).
- **UI:** `/challenges` (active/past list), `/challenges/new?to=` (propose form: ruleset + time + GymPicker host + risk-acknowledgement checkbox), `/challenges/[id]` (terms, negotiation, waiver sign, host-admin approve, run-on-console links). Gym waiver CRUD at `POST/GET /api/admin/waivers` (versioned, supersedes prior active).
- **Verified (Playwright, 3 users + DB):** challenger sees Challenge button → propose → challenged accepts + signs → challenger signs → status gym_pending → host admin approves → scheduled → admin starts console → Finish (Test Student wins by submission) → challenge `completed`, `winnerId`=challenger, 2 waiver sigs, table `done`, S1 record **1W–0L**. All test data + the temp Ascend hosting flags / admin-gym change cleaned up.

**Scoreboard TV mode — DONE 2026-06-12:** the public `/scoreboard/[slug]` now renders with NO app header/footer (via `app/components/LayoutChrome.tsx`, which strips chrome on `/scoreboard/*`) and has a **fullscreen toggle button** (⛶ top-right, Fullscreen API) for true TV display. Browser-verified. tsc + build clean. Uncommitted.

### Phase 58 Milestone 2 — DONE & browser-verified 2026-06-12 (NOT pushed to git)
Tournaments now drive the live console. tsc + `npm run build` clean. Playwright-verified end to end: admin → tournament Brackets tab → **"Run on console"** on a pending match opens `/console/[tableId]` (competitors auto-filled from the bracket) → run clock + score → **Finish (Test Student wins by submission)** → bracket reloads with **"Top wins"** + Console/Scoreboard links. DB confirmed: `TournamentMatch.result=participant_a_wins`, tournament `status=complete` (final), `MatchTable.winnerSide=a winBy=submission status=done`, notes "Console: by submission". (Test data + temp admin-gym change fully cleaned up.)
- **Shared propagation** extracted to `lib/tournamentResult.ts` (`applyTournamentMatchResult`) — used by BOTH the manual result editor (`PUT .../matches/[matchId]`) and the console Finish route (no duplicated bracket logic).
- **New route** `POST /api/admin/tournaments/[id]/matches/[matchId]/table` — idempotent; creates a `MatchTable` linked via `tournamentMatchId`/`tournamentId`, competitor names resolved from registrations, ruleset snapshot (default ibjjf_gi). Verifies match belongs to the tournament + both competitors set.
- **Finish route** (`/api/tables/[id]/finish`) now propagates to the bracket when `tournamentId`+`tournamentMatchId` are set (winnerSide→MatchResult; draw supported).
- **Bracket UI** (`BracketView` MatchCard, both single-elim + round-robin): pending matches w/ both competitors show **"Run on console"**; once a table exists, show **Console ↗ / Scoreboard ↗** links. Manage page + `GET /api/admin/tournaments/[id]` return a `tableByMatch` map so links survive refresh.

### Phase 58 Milestone 1 — DONE & browser-verified 2026-06-10/12 (NOT pushed to git)
Standalone single-match console; db push run, dev restarted. Playwright-verified: anchored clock stays in sync across console + public scoreboard (1s), score/undo/Finish all correct (DB source-of-truth confirmed).
- **Schema** (additive, in `prisma/schema.prisma`): `MatchTable` (anchored clock fields `clockStatus`/`remainingMs`/`anchorTs`/`periodMs`/`period`, denormalized competitor names, `publicSlug`, `rulesetConfig` snapshot), `MatchTableAssignment`, `MatchScoreEvent` (append-only, `voided` for undo), enum `MatchTableStatus`. No User-model changes (assignee/creator stored as raw `userId` strings).
- **Authoritative anchored clock** implemented exactly per guide: timekeeper's client stamps the press instant in server time (`estServerNow = Date.now() − offset`) → sent as `anchorTs` + recomputed `remainingMs`, stored verbatim. Every API response returns `serverNow`; clients compute skew `offset` (with rtt/2) and derive display as `remainingMs − (estServerNow − anchorTs)`. 1s poll + 100ms local interpolation tick. Optimistic local patch on press, then resync.
- **`lib/rulesets.ts`** — web-verified (2026-06) presets: IBJJF Gi/No-Gi/Masters (TD 2, sweep 2, KOB 2, pass 3, mount 4, back 4; adult times white5/blue6/purple7/brown8/black10), ADCC (clean TD 4, sweep 2, clean sweep 4, pass 3, KOS 2, mount 2, back 3; negative pts), Submission-only/EBI, Custom. Official URLs: ibjjf.com/news/new-rules-updates, adcombat.com/adcc-rules-regulations. Snapshotted onto the table on create.
- **API:** `POST/GET /api/tables`, `GET /api/tables/[id]` (control state + serverNow + caps), `POST .../clock` (timekeeper-stamped start/stop/adjust/period), `POST .../score` + `POST .../score/[eventId]/void`, `POST .../finish` (manual winner; bracket/challenge propagation deferred to M2), `POST/DELETE .../assign[/userId]`, public `GET /api/scoreboard/[slug]`. Control auth via `lib/tableAuth.ts` (creator / assignee / gym-admin / site_admin). Score reducer + clock derive in `lib/matchScore.ts`.
- **UI:** `/console` (index of your tables), `/console/new` (create form w/ ruleset summary + Full-rules link), `/console/[tableId]` (combined operator: big clock + controls, per-side scoring from ruleset, advantage/penalty, events log w/ undo, Finish panel, scoreboard QR + public link), public `/scoreboard/[slug]` (TV-scaled vh units, login-free). Generic `app/components/QRCode.tsx` (`QRCodeUrl`). Middleware: `/scoreboard/` + `/api/scoreboard/` made public.
- **No nav entry yet** — reach it directly at `/console`. (Add a Header link or wire from tournaments in M2.)
...
### ⚠️ Ops / state to know
- **Phase 59 `prisma db push` (2026-06-12) went to the SHARED cluster (live):** `ChallengeMatch`, `ChallengeTermsHistory`, `GymWaiver`, `WaiverSignature` + `ChallengeStatus`/`WaiverKind` enums; `User.competeTournaments`/`acceptsChallenges`; `Gym.hostsInHouse/OpenChallenges`. Additive — no data loss.
- **Phase 58 `prisma db push` (2026-06-12) went to the SHARED cluster (live):** `MatchTable`, `MatchTableAssignment`, `MatchScoreEvent` + `MatchTableStatus` enum. Additive — no data loss. Dev server already restarted.
- **TEST-ONLY change live in code (NOT a DB change):** tournament-creation participating-gym gate is bypassed in `app/api/admin/tournaments/route.ts` + `app/admin/tournaments/page.tsx` (marked `TEST ONLY (Phase 58)`). **Revert before launch.**
- **Leftover demo row in DB:** `MatchTable` labeled `Mat A — Demo` (seeded for UI preview). Harmless; delete anytime.
- **`admin@gym.com` has `gymId = null`** — it can't own tournaments. The account the user actually manages with is a different admin on gym `cmq56bwfl000004l2oywi1mls`. (For the M2 test I temporarily set admin's gym then restored it to null.)
- **Earlier additive pushes (prior sessions):** `GymClaim`, `Invitation.email/meta`, `ProviderStatus`+`User.provider*`, `InstructorReview`+`User.ratingAvg/ratingCount`. All additive.
- **Nothing pushed to git or deployed** — Phase 58 M1+M2 are uncommitted working-tree changes on `main`. User handles git + push/deploy.
...
### Other open work (not blocking Phase 59)
...
- [x] **Prominence (2026-06-30):** verified in code — DMs (envelope icon, NavBadges, desktop+mobile), Chats + Forums (text nav links), Notifications (bell). All ≤1 tap. DM entry is icon-only (no text label) — acceptable; revisit if testers miss it.
- [x] **Empty states (2026-06-30):** `/messages` empty state rewritten to explain purpose + point to "New Message"; `/forum` (SIMPLE_LAUNCH-aware → group chats) and `/chats` ("start one above") already explain. 
- [ ] **DM + forum behavioral audit (needs runtime/browser):** request/approval flow, student-DM opt-out, instructor-always-allowed, gym-scoping, blocked-user handling, realtime delivery; forum threaded replies, pinning, galleries, subscribe/auto-subscribe, moderation.

### 62.2 — Onboarding correct + intuitive for ALL roles (member · private instructor · gym/gym-admin)
- [~] **Member wizard audited (2026-06-30):** solid + pivot-aware. Fixed 2 simple-mode issues: (1) "Add my gym" no longer dumps the user out of onboarding (routed to blocked `/gyms/register`) — now shows an inline note + lets them continue independently; (2) Step-5 "schedule" → "your training schedule" (accurate now schedule=My Training). Owner wizard is hidden in simple mode (middleware). ⚠️ Private/independent-instructor onboarding path still needs a walk-through.
  - ⏸ **PARKED DECISION:** in simple mode, should members be able to **add a missing gym to the directory** (for teammate-location)? `/gyms/register` is hidden (owner-oriented), so new gyms currently can't be added by members. Options: (a) keep hidden — members can only pick existing gyms; (b) add a lightweight "add gym to directory" path (no management) reachable in simple mode. Recommend (b) if teammate-location matters for gyms not yet listed.
- [x] **Private-instructor path audited (2026-07-02):** end-to-end flow is coherent — member onboarding → discover on `/lessons` → apply at `/provider` (thorough form) → approval notification ("You can now offer private lessons") → post-approval orientation (amber nav link + dashboard card + `/provider` dashboard w/ availability + requests). Fixed the `/lessons` apply CTA: was a tiny gray unconditional line that showed "Apply…" even to already-approved providers — now a **state-aware bordered card** (none→Apply, rejected→Reapply, pending→"in review", approved→"manage your lessons"). ⚠️ Safety: the parked "convicted of a violent/sexual offense?" question is still NOT on the application (form asks arrests-in-5yr only) — needs your go-ahead.
...
No invite system exists today. **Guide:** `guides/phase40-invitations.md` (design + confirmed decisions, 2026-06-07). **Focus next:** 40.1/40.3/40.4 (individual links). **Decisions:** friend link = one reusable permanent per-user link → mutual follow on signup; gym-member invite = auto-active + follow admin; instructor invite = single-use link OR approve-after-registration (admin picks); already-a-member → "already registered" + login. Building blocks exist: `Follow` model + `/api/users/[id]/follow`; `GymMembership.status` has `pending`.

### [~] 40.1 — Individual invite: link + QR DONE (email deferred). Any user → reusable friend link at `/invite` (dashboard "Invite Friends" tile), mutual-follow on signup. Gym admin on `/admin/users`: **Invite a Member** (reusable, auto-active) + **Invite an Instructor** (single-use link OR approve-after, with pending-requests list). Already-registered users: invite applies on login (`/login?invite=`) or via Accept on `/invite/[token]`. `Invitation`/`InviteRedemption` models + `User.instructorRequestedAt` (pushed); `lib/invites.ts`; `/api/invites`(GET friend/POST gym) + `/api/invites/[token]/accept`. **Remaining in 40.1:** email delivery (deferred to gym-membership onboarding). **NOT browser-tested.**
**Also done (2026-06-07): "student"→"Member" display sweep** — role labels everywhere + visible instructor/admin/help copy. Left: internal `student` role value, routes, "Student Notes" feature name.
### [x] 40.2 — Bulk CSV import — DONE 2026-06-09. `/admin/import` (linked from `/admin/users`): upload any CSV → client-side parser → **editable column mapping** (auto-guessed; map to Email*/Name/Belt/Phone/Instructor/Notes, rest ignored) → **preview** (valid/skipped counts) → `POST /api/admin/invites/bulk`. Additive schema: `Invitation.email` + `Invitation.meta` (prefill) + `@@index([email])`. Capped 1000 rows; belt normalized; instructor parsed (yes/y/true/1/coach).
### [x] 40.3 — Acceptance / pre-association — DONE. Bulk import: **existing accounts** associated immediately (active membership + gymId-if-null + prefill empty name/belt/phone + mutual-follow admin); **new emails** get a targeted `Invitation` that auto-applies on signup via `applyEmailInvites` (hooked into `POST /api/auth/register`). Already-active members skipped. (Switch-gym for users with a different gym = associate via additional membership; gymId only set when null — no forced switch.)
### [x] 40.4 — Instructor-flagged rows — DONE. Instructor=yes rows **never auto-grant**: existing users get `instructorRequestedAt` set; new-email invites are `gym_instructor` with `grantOnAccept=false` (approve-after). Owner approves in the existing `/admin/users` Instructor Requests list.
### [x] 40.5 — Gym-scoped invites — DONE. All bulk invites carry `gymId = admin's gym`; the bulk endpoint is admin-gated to the caller's gym. Invite landing already shows gym context (40.1).

**Phase 40 COMPLETE.** Browser-verified (Playwright + DB, 20/20: CSV upload→map→preview→import; existing-associate w/ prefill; instructor-request; targeted invite; new-email signup auto-applies). Live email delivery still deferred (Resend sandbox — beta blocker 47.1); links/QR/auto-apply work regardless.
...
### [x] 42.2 — Request UX: DONE — `/lessons/new` is slot-based (instructor → fetch `/api/instructors/[id]/availability` → 8-week multi-select calendar → one PrivateLesson per slot; free datetime fallback for instructors w/o availability).
### [x] 42.3 — Home-gym vs radius search: DONE (see above — `/lessons` lists home-gym instructors by default + `InstructorSearch` radius for outside-org).
### [x] 42.6 — Re-geocode on site-admin gym address edit — DONE 2026-06-09. `PUT /api/site-admin/gyms/[id]` now also accepts **`address`** (was silently dropped) and **re-geocodes** lat/lng (best-effort) whenever address/city/state/zip changes, so radius search stays accurate after edits. Browser+DB verified (4/4; White House address → 38.898,-77.037).
### [x] 42.4 — Independent (non-gym) providers — DONE 2026-06-09. Guide: `guides/DO_NOT_REVIEW/phase42-4-independent-providers.md`. Additive schema (`ProviderStatus` enum + `User.providerStatus/providerApprovedById/providerApprovedAt/providerBio/providerCity/providerState/providerLat/providerLng`). `/provider` apply page (`POST /api/provider/apply` → pending + geocode location + notify approvers); approval queue `/provider/approvals` for **verified black belts + site_admins** (`GET /api/provider/applications`, `PATCH /api/provider/applications/[id]`); on approve → can edit availability (relaxed `/api/instructor/availability` gate via `lib/provider.ts canEditAvailability`) at `/provider/availability` + appears in `/api/instructors/search` radius results labelled "Independent" (own geocoded coords). Entry link on `/lessons`. **Browser+DB verified 14/14** (apply→pending+geocode, availability blocked pre-approval, blue-belt can't approve, black-belt approves→notify, provider sets availability + is discoverable).
...
### Phase 58 — Live Match Console (scoring + timer + scoreboard) — GUIDE WRITTEN 2026-06-10
**Guide:** `guides/phase58-live-match-console.md`. Reusable single-match console (scorer adds points, timekeeper runs the clock, public login-free **scoreboard URL + QR**). **Authoritative anchored clock** (timekeeper's press instant = truth; everyone derives from one server anchor → zero drift; polling ~1s + local interpolation + clock-skew offset; no realtime infra). Tables assigned via "share with…" user search. Append-only `MatchScoreEvent` log (undo/correct/audit). `lib/rulesets.ts` (IBJJF Gi/No-Gi/Masters, ADCC, sub-only, Custom — **verify exact points/URLs via web research at build time**). Used by tournaments (Phase 32, N tables, propagate winner to bracket) + challenges (Phase 59). **Build this FIRST**; milestone 1 = standalone single-match console. NOT built.

### Phase 59 — Full 'Scend Challenge Matches — GUIDE WRITTEN 2026-06-10
**Guide:** `guides/phase59-challenge-matches.md`. Depends on Phase 58 + Phase 43 waiver plumbing (built here). Member opt-ins (`competeTournaments`/`acceptsChallenges`; onboarding + settings; Challenge button next to Follow on profile when opted in). Gym hosting opt-ins (`hostsInHouseChallenges`/`hostsOpenChallenges`) + hosting directory (`/gyms?hosting=challenges`) + uploaded **visitor/challenge waiver** (gym supplies text; typed-name e-sign). `ChallengeMatch` negotiation state machine (propose→counter↔→accept→sign waivers→host-gym approve→schedule→run via console→result). Optional events-calendar listing. **Combined competition record** on profile. ⚖️ **ToS must add** gym-liability + hold-harmless-AscendIt language (ties to Phase 43/47.2). NOT built.
