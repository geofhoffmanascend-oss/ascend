...
### [ ] 42.4 — [DEFERRED — needs user go-ahead] Individual (non-gym) private-lesson providers must be **approved by a verified black belt** before they can offer lessons. v1 was intentionally gym-instructors-only; this is a net-new trust/approval system (see ⏸ DECISIONS PARKED).

**Phase 42 functionally complete** (availability, slots, radius search, geocode-on-edit). Only 42.4 (independent providers) remains, deferred. Ops: add `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` to Vercel for prod geocoding; modern vector map UI (gym-finder/events-near-me) = backlog.
