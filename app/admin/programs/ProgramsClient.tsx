...
type Program = { id: string; name: string; description: string | null; classCount: number }
...
export function ProgramsClient({ initial }: { initial: Program[] }) {
  ...
  async function create(e: React.FormEvent) {
    ...
    try {
      ...
      setPrograms(ps => [...ps, { id: data.program.id, name: data.program.name, description: data.program.description, classCount: 0 }])
  ...
  async function saveEdit(id: string) {
    if (!editName.trim()) { setError('Program name is required'); return }
    setSaving(true)
    setError('')
    try {
  ...
  async function remove(p: Program) {
    const msg = p.classCount > 0
      ? `Delete "${p.name}"? Its ${p.classCount} class${p.classCount !== 1 ? 'es' : ''} will stay but become ungrouped.`
      : `Delete "${p.name}"?`
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        {programs.length === 0 && <p className="text-ash text-sm italic">No programs yet. Create one below.</p>}
        {programs.map(p => (
          <div key={p.id} className="border border-smoke bg-paper p-4">
            {editing === p.id ? (
              <div className="flex flex-col gap-3">
                <input value={editName} onChange={e => setEditName(e.target.value)} className={inputCls} placeholder="Program name" />
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-ash mt-1">{p.classCount} class{p.classCount !== 1 ? 'es' : ''}</p>
                </div>
      ...
      <form onSubmit={create} className="border border-smoke bg-paper p-5 flex flex-col gap-4">
        <p className="font-display text-lg text-ink">New Program</p>
