...
export default async function AdminUsersPage({
  ...
  // Pending instructor requests (from "approve-after" instructor invites).
  const instructorRequests = gymId
    ? await prisma.user.findMany({
        where: { gymId, instructorRequestedAt: { not: null }, NOT: { roles: { has: 'instructor' } } },
        select: { id: true, name: true, email: true },
        orderBy: { instructorRequestedAt: 'asc' },
      })
    : []
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      {/* Invite an instructor + pending requests */}
        ...
        {instructorRequests.length > 0 && (
          <div className="border border-brand-red bg-paper p-5">
            ...
            <div className="flex flex-col gap-2">
              {instructorRequests.map(u => (
                <Link key={u.id} href={`/admin/users/${u.id}`} className="flex items-center justify-between border border-smoke bg-paper px-4 py-2.5 hover:border-steel transition-colors">
                  <div>
                    <p className="text-sm font-medium text-ink">{u.name ?? '(no name)'}</p>
                    <p className="text-xs text-ash">{u.email}</p>
                  </div>
                  <span className="text-xs font-semibold text-brand-red">Review →</span>
                </Link>
              ))}
            </div>
            <p className="text-xs text-ash mt-3">Grant the instructor role from a member's page to approve.</p>
          </div>
        )}
      </div>
