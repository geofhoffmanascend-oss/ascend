...
export default async function ProviderLessonsPage() {
  const session = await getServerSession(authOptions)
  if (!session?.user?.id) redirect('/login?callbackUrl=/provider/lessons')
  ...
  function Row({ lesson }: { lesson: typeof lessons[number] }) {
    return (
      <Link href={`/lessons/${lesson.id}`} className="border border-smoke bg-paper p-4 hover:border-steel transition-colors flex items-center justify-between">
        <div>
        ...
        <span className={`px-2 py-0.5 text-xs font-bold uppercase tracking-wide flex-shrink-0 ml-4 ${STATUS_STYLES[lesson.status]}`}>{lesson.status}</span>
      </Link>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        <div className="inline-block bg-brand-red px-3 py-1 mb-3"><span className="font-display text-xs font-bold tracking-widest uppercase text-paper">Private Instructor</span></div>
      ...
      <div className="flex flex-col gap-6">
        {pending.length > 0 && (
          <section>
            ...
            <div className="flex flex-col gap-2">{pending.map(l => <Row key={l.id} lesson={l} />)}</div>
          </section>
        )}
        {confirmed.length > 0 && (
          <section>
            <p className="text-xs font-bold uppercase tracking-widest text-steel mb-3">Confirmed</p>
            <div className="flex flex-col gap-2">{confirmed.map(l => <Row key={l.id} lesson={l} />)}</div>
          </section>
        )}
        {past.length > 0 && (
          <section>
            <p className="text-xs font-bold uppercase tracking-widest text-steel mb-3">Past</p>
            <div className="flex flex-col gap-2">{past.map(l => <Row key={l.id} lesson={l} />)}</div>
          </section>
        )}
        {lessons.length === 0 && <p className="text-ash text-sm italic">No lesson requests yet.</p>}
      </div>
    </div>
  )
}
